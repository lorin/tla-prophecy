...
D3:   j := 1;
D4:   while(j<=range) do
D5:   await q.items[j] = null \/ q.items[j] = Head(p);
      q.items[j] := null || y := q.items[j];
D6:   if(y /= null) then
D7:       rval[self] := y;
          p := Tail(p);
D8:       return;
...
VARIABLES pc, op, arg, rval, done, q, p, stack, x, i, j, y, range

vars == << pc, op, arg, rval, done, q, p, stack, x, i, j, y, range >>
...
E1(self) == /\ pc[self] = "E1"
            ...
            /\ UNCHANGED << op, arg, rval, done, p, stack, x, j, y, range >>

E2(self) == /\ pc[self] = "E2"
            /\ q' = [q EXCEPT !.items[i[self]] = x[self]]
            /\ pc' = [pc EXCEPT ![self] = "E3"]
            /\ UNCHANGED << op, arg, rval, done, p, stack, x, i, j, y, range >>
...
E3(self) == /\ pc[self] = "E3"
            ...
            /\ UNCHANGED << op, arg, rval, done, q, p, j, y, range >>

Enq(self) == E1(self) \/ E2(self) \/ E3(self)

D1(self) == /\ pc[self] = "D1"
            /\ pc' = [pc EXCEPT ![self] = "D2"]
            /\ UNCHANGED << op, arg, rval, done, q, p, stack, x, i, j, y,
                            range >>
...
D3(self) == /\ pc[self] = "D3"
            ...
            /\ UNCHANGED << op, arg, rval, done, q, p, stack, x, i, y, range >>
...
D5(self) == /\ pc[self] = "D5"
            /\ q.items[j[self]]=null \/ q.items[j[self]] = Head(p)
            /\ /\ q' = [q EXCEPT !.items[j[self]] = null]
               /\ y' = [y EXCEPT ![self] = q.items[j[self]]]
            /\ pc' = [pc EXCEPT ![self] = "D6"]
            /\ UNCHANGED << op, arg, rval, done, p, stack, x, i, j, range >>
...
D7(self) == /\ pc[self] = "D7"
            ...
            /\ UNCHANGED << op, arg, done, q, p, stack, x, i, j, y, range >>
...
D8(self) == /\ pc[self] = "D8"
            ...
            /\ stack' = [stack EXCEPT ![self] = Tail(stack[self])]
            /\ UNCHANGED << op, arg, rval, done, q, p, x, i >>
...
enqdone(self) == /\ pc[self] = "enqdone"
                 ...
                 /\ UNCHANGED << op, arg, rval, q, p, stack, x, i, j, y, range >>
